...
function handlePost($action, $auth, $wiki, &$message, &$messageType) {
    ...
    switch ($action) {
        case 'login':
            ...
            break;
        case 'register':
            $result = $auth->register($_POST['username'], $_POST['password'], $_POST['role'] ?? 'user');
            if ($result['success']) {
        ...
        case 'create':
            ...
            $result = $wiki->createPage($_POST['title'], $_POST['content'], $tags);
            if ($result['success']) {
                header('Location: /?action=view&page=' . urlencode($_POST['title']));
                exit;
            } else {
                $message = $result['message'];
                $messageType = 'error';
...
';
        foreach ($recentPages as $page) {
            echo '<li>';
            echo '<a href="/?action=view&page=' . urlencode($page['title']) . '">' . htmlspecialchars($page['title']) . '</a>';
            echo '<div class="page-meta">Updated ' . date('M j, Y', strtotime($page['updated_at'])) . ' by ' . htmlspecialchars($page['author']) . '</div>';
            echo '</li>';
        }
        echo '</ul>';
    } else {
...
function showPage($wiki, $title) {
    ...
    if (!$page) {
        ...
        echo '<p>The page "' . htmlspecialchars($title) . '" does not exist.</p>';
    ...
    if (!empty($page['tags'])) {
        ...
        foreach ($page['tags'] as $tag) {
            $tagLinks[] = '<a href="' . createTagSearchUrl($tag['name']) . '" class="tag-link-inline">' . htmlspecialchars($tag['name']) . '</a>';
        }
    ...
    if ($auth->isLoggedIn()) {
        ...
        echo '<a href="/?action=edit&page=' . urlencode($title) . '" class="btn">Edit Page</a>';
        echo '<a href="/?action=view&page=' . urlencode($title) . '&revisions=1" class="btn">View History</a>';
        echo '<a href="#" class="btn btn-danger" onclick="if(confirm(\'Delete this page?\')) window.location=\'?action=delete&page=' . urlencode($title) . '\'">Delete Page</a>';
        echo '</div>';
    }
}
...
function showEditForm($wiki, $auth, $title) {
    ...
    echo '<input type="text" name="tags" id="tags" value="' . htmlspecialchars($currentTags) . '" placeholder="documentation, tutorial, guide">';
    echo '<small class="text-light">Separate multiple tags with commas</small>';
    echo '</div>';
    echo '<div class="form-group">';
    echo '<label for="content">Content (Markdown):</label>';
    echo '<textarea name="content" id="content" required>' . htmlspecialchars($page['content'] ?? '') . '</textarea>';
    echo '</div>';
    echo '<button type="submit" class="btn">Save Changes</button>';
    echo '<a href="/?action=view&page=' . urlencode($title) . '" class="btn">Cancel</a>';
    echo '</form>';
}
...
function showPageList($wiki) {
    ...
    if (empty($pages)) {
        ...
        foreach ($pages as $page) {
            echo '<li><a href="/?action=view&page=' . urlencode($page['title']) . '">' . htmlspecialchars($page['title']) . '</a></li>';
        }
        echo '</ul>';
    }
}
...
function showSearchForm() {
    ...
    echo '<input type="text" name="q" id="quick-q" value="' . htmlspecialchars($query) . '" placeholder="Search pages..." class="search-input">';
    ...
    echo '<input type="text" name="q" id="adv-q" value="' . htmlspecialchars($query) . '" placeholder="Keywords, phrases..." class="filter-input">';
    ...
    foreach ($allTags as $tag) {
        ...
        echo '<input type="checkbox" name="tags[]" value="' . htmlspecialchars($tag['name']) . '" ' . $checked . '>';
        echo '<span class="tag-label">' . htmlspecialchars($tag['name']) . '</span>';
        echo '</label>';
    }
    echo '</div>';
    echo '</div>';
    ...
    echo '<input type="date" name="date_from" id="date_from" value="' . htmlspecialchars($dateFrom) . '" class="filter-input">';
    echo '</div>';
    echo '<div class="date-input-group">';
    echo '<label for="date_to" class="date-label">To:</label>';
    echo '<input type="date" name="date_to" id="date_to" value="' . htmlspecialchars($dateTo) . '" class="filter-input">';
    echo '</div>';
    echo '</div>';
    echo '</div>';
    ...
    foreach ($allAuthors as $authUser) {
        ...
        echo '<option value="' . htmlspecialchars($authUser['username']) . '" ' . $selected . '>' . htmlspecialchars($authUser['username']) . '</option>';
    }
    echo '</select>';
    echo '<div class="author-selector" id="author-selector">';
    $allOptionSelected = empty($author) ? 'selected' : '';
    echo '<div class="author-option ' . $allOptionSelected . '" data-value="">All Authors</div>';
    foreach ($allAuthors as $authUser) {
        $selected = ($author === $authUser['username']) ? 'selected' : '';
        echo '<div class="author-option ' . $selected . '" data-value="' . htmlspecialchars($authUser['username']) . '">' . htmlspecialchars($authUser['username']) . '</div>';
    }
    echo '</div>';
    echo '<input type="hidden" name="author" id="selected-author" value="' . htmlspecialchars($author) . '">';
    echo '</div>';
    echo '</div>';
    ...
    $activeFilters = [];
    if ($query) $activeFilters[] = ['type' => 'query', 'label' => 'Query', 'value' => "\"$query\""];
    if (!empty($selectedTags)) $activeFilters[] = ['type' => 'tags', 'label' => 'Tags', 'value' => implode(', ', $selectedTags)];
    if ($dateFrom) $activeFilters[] = ['type' => 'date_from', 'label' => 'From', 'value' => $dateFrom];
    if ($dateTo) $activeFilters[] = ['type' => 'date_to', 'label' => 'To', 'value' => $dateTo];
    if ($author) $activeFilters[] = ['type' => 'author', 'label' => 'Author', 'value' => $author];
    ...
    if (!empty($activeFilters)) {
        ...
        foreach ($activeFilters as $filter) {
            ...
            echo '<strong>' . htmlspecialchars($filter['label']) . ':</strong> ' . htmlspecialchars($filter['value']);
            echo '<a href="' . removeFilterFromUrl($filter['type']) . '" class="filter-remove" title="Remove this filter">×</a>';
            echo '</span>';
        }
        echo '</div>';
        echo '</div>';
    ...
    if (($query || !empty($filters)) && !empty($results)) {
        ...
        echo '<span class="results-count">' . count($results) . ' pages found</span>';
        echo '</div>';
        ...
        foreach ($results as $result) {
            ...
            echo '<h3><a href="/?action=view&page=' . urlencode($result['title']) . '">' . htmlspecialchars($result['title']) . '</a></h3>';
            echo '<div class="result-meta">';
            echo '<span class="result-author"><i class="fas fa-user"></i> ' . htmlspecialchars($result['author']) . '</span>';
            echo '<span class="result-date"><i class="fas fa-calendar"></i> ' . date('M j, Y', strtotime($result['updated_at'])) . '</span>';
            echo '</div>';
            echo '</div>';
            ...
            if (!empty($result['content'])) {
                ...
                echo '<div class="result-preview">' . htmlspecialchars($preview) . '</div>';
            }

            echo '<div class="result-actions">';
            echo '<a href="/?action=view&page=' . urlencode($result['title']) . '" class="btn btn-sm">View Page</a>';
            echo '</div>';
            echo '</div>';
        }
        echo '</div>';
    ...
    if (!empty($allTags)) {
        ...
        foreach (array_slice($allTags, 0, 15) as $tag) {
            ...
            echo '<a href="' . addTagToUrl($tag['name']) . '" class="' . $tagClass . '">' . htmlspecialchars($tag['name']) . '</a>';
        }
        echo '</div>';
        echo '</div>';
        echo '</div>';
...
function showRevisions($wiki, $auth, $title) {
    ...
    if (empty($revisions)) {
        ...
        echo '<p><a href="/?action=view&page=' . urlencode($title) . '" class="btn">Back to Page</a></p>';
        return;
    }

    echo '<h1>Page History: ' . htmlspecialchars($title) . '</h1>';
    echo '<p><a href="/?action=view&page=' . urlencode($title) . '" class="btn">Back to Current Page</a></p>';
    ...
    foreach ($revisions as $index => $revision) {
        ...
        echo '<div class="revision-item' . ($isCurrent ? ' current' : '') . '">';
        echo '<div class="revision-header">';
        echo '<div class="revision-meta">';
        echo '<span class="revision-version">Version ' . $version . '</span>';
        echo '<span class="revision-author"><i class="fas fa-user"></i> ' . htmlspecialchars($revision['author']) . '</span>';
        echo '<span class="revision-date"><i class="fas fa-calendar"></i> ' . date('M j, Y H:i', strtotime($revision['timestamp'])) . '</span>';
        if ($isCurrent) {
            echo '<span class="revision-current">Current</span>';
        }
        echo '</div>';
        echo '<div class="revision-actions">';
        echo '<a href="/?action=view&page=' . urlencode($title) . '&revision=' . $revision['id'] . '" class="btn btn-sm">View</a>';
        if (!$isCurrent && $auth->isLoggedIn()) {
            echo '<a href="/?action=restore&revision=' . $revision['id'] . '&page=' . urlencode($title) . '" class="btn btn-sm btn-warning" onclick="return confirm(\'Are you sure you want to restore this version?\')">Restore</a>';
        }
        echo '</div>';
        echo '</div>';
        ...
        echo '<div class="revision-preview">' . htmlspecialchars($preview) . '</div>';
        echo '</div>';
    }
    echo '</div>';
}
...
function showRevision($wiki, $revisionId) {
    ...
    echo '<p><strong>Author:</strong> ' . htmlspecialchars($revision['author']) . '</p>';
    echo '<p><strong>Date:</strong> ' . date('M j, Y H:i', strtotime($revision['timestamp'])) . '</p>';
    echo '</div>';

    echo '<div class="page-content">' . $revision['rendered_content'] . '</div>';

    echo '<div class="page-actions">';
    echo '<a href="/?action=view&page=' . urlencode($revision['title']) . '&revisions=1" class="btn">Back to History</a>';
    echo '<a href="/?action=view&page=' . urlencode($revision['title']) . '" class="btn">Back to Current Page</a>';
    if ($auth->isLoggedIn()) {
        echo '<a href="/?action=restore&revision=' . $revisionId . '&page=' . urlencode($revision['title']) . '" class="btn btn-warning" onclick="return confirm(\'Are you sure you want to restore this version?\')">Restore This Version</a>';
    }
    echo '</div>';
}
...
function handleRestore($wiki, $auth) {
    ...
    if ($result['success']) {
        header('Location: /?action=view&page=' . urlencode($pageTitle) . '&message=Revision restored successfully');
    } else {
        header('Location: /?action=view&page=' . urlencode($pageTitle) . '&error=Failed to restore revision');
    }
    exit;
}
?>